    ...
    try:
        ...
        for position in open_positions:
            ...
            if symbol_2 not in symbols_list:
                ...
                print('\n----------')
                print(symbol_2)
                print(f'{round(unrealised_pnl_, 3)}, {round(wallet_balance, 3)}')
                print(f'{round((unrealised_pnl_ / wallet_balance) * 100, 3)}%')


                if ((unrealised_pnl_ / wallet_balance) * 100 >= 13) or ((unrealised_pnl_ / wallet_balance) * 100 < -12):  # 이익이 3% 이상이거나 손실이 3% 이하인 경우
                    ...
                    if ((unrealised_pnl_ / wallet_balance) * 100 >= 13):
                        ms = 'Closed Engin: 이익이 13% 이상 이므로 익절, 포지종료.'
                    elif ((unrealised_pnl_ / wallet_balance) * 100 < -12):
                        ms = 'Closed Engin: 손실이 -12% 이상 이므로 손절, 포지종료.'
                        
                    print(f"{symbol_}: {ms}")
                    send_to_telegram(f"{symbol_}: {ms}")

            # else:

            #     if ((unrealised_pnl_ / wallet_balance) * 100 >= 13) or ((unrealised_pnl_ / wallet_balance) * 100 < -14):  # 이익이 3% 이상이거나 손실이 3% 이하인 경우
            ...
            #         if ((unrealised_pnl_ / wallet_balance) * 100 >= 13):
            #             ms = 'Running Engin: 이익이 13% 이상 이므로 익절, 포지종료.'
            #         elif ((unrealised_pnl_ / wallet_balance) * 100 < -14):
            #             ms = 'Running Engin: 손실이 -14% 이상 이므로 손절, 포지종료.'
